...
# Debugging: Print paths to ensure they are set correctly
...
if [ -f "$bam_file" ]; then
    ...
    # Check for errors during the alignment
    if [ $? -ne 0 ]; then
...
#----- STEP 5 - paralogues searching step usin R script-----
#echo "[ Step 5/10 ] : Rscript to get paralogues genes"

#start_time=$(date +%s)
#PARALOGUE_FILE="$OUTPUT/paralogue_gene.txt"

#if [ ! -f "$PARALOGUE_FILE" ]; then
#    echo "Running the [ Step to find Paralogue genes in Arabidopsis thaliana..."
    #Rscript $SCRIPT_DIR/removeParaologue.R "$ANNOT_FILE" "$TRANS_FILE" "$OUTPUT/paralogue_gene.txt"
#    Rscript scripts_2/removeParaologue.R "$ANNOT_FILE" "$TRANS_FILE" "$OUTPUT/paralogue_gene.txt"
#else
#    echo "Paralogues gene file already exists. Skipping the generation [ Step."
#fi
#end_time=$(date +%s)
#echo "Finding paralogue genes took $((end_time - start_time)) seconds."


#----- STEP 6 - removal of paralogues gene fusion pair -----
...
# Run chimera_breakpoint_detecter.py
echo "[ Step 8/10 ] : chimera_breakpoint_detecter.py"
if [ ! -f $bp_detector_out ]; then

    #python $PWD/scriptsGKscript/chimera_breakpoint_detecter.py "$OUTPUT_DIR/${$SAMPLE_OUTPUT}_df_blast_output.txt" 10 5 "$OUTPUT_DIR/${$SAMPLE_OUTPUT}_allsplit.txt"  15 150 20 0
...
if [ ! -f $chimera_iden ]; then 
    ...
    #python ../$SCRIPT_DIR/chimera_identifier.py ../input/arab/Arabidopsis_thaliana.TAIR10.56.gtf transcript_info.csv exons_position_adjusted.csv 272_allsplit.txt
    start_time=$(date +%s)  
...
start_time=$(date +%s)
python $SCRIPT_DIR/chimera_filter.py $OUTPUT_DIR/${$SAMPLE_OUTPUT}_chimera_identified.csv $JunctionDist $ShanEnt_Seq $REFERENCE_FILE $JunctionSeq $BptDinucleotide $TopHits $Promiscioushits $split $span $OUTPUT_DIR/${$SAMPLE_OUTPUT}_filtered_chimera.csv
if [ $? -ne 0 ]; then
...
echo ""
echo "Script completed successfully. Output file saved in "$OUTPUT_DIR/${$SAMPLE_OUTPUT}_filtered_chimera.csv" directory"
